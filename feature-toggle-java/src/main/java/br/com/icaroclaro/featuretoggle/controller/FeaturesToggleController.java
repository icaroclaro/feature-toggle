...
import br.com.icaroclaro.featuretoggle.dto.FeatureToggleResponse;
import br.com.icaroclaro.featuretoggle.model.Feature;
import br.com.icaroclaro.featuretoggle.model.FeatureToggle;
import br.com.icaroclaro.featuretoggle.repository.FeatureToggleRepository;

@RestController
@RequestMapping("/featureToggles")
public class FeaturesToggleController {
	@Autowired
	private FeatureToggleRepository repository;
	
	@GetMapping
	public ResponseEntity<List<FeatureToggleResponse>>consultaFeatureToggles() {
		List<FeatureToggle> featureToggles = repository.findAll();
		if(featureToggles == null)
			return ResponseEntity.notFound().build();
		
		List<FeatureToggleResponse> response = FeatureToggleResponse.convertToListFeatureToggleResponse(featureToggles);			
